...
Game.prototype.update = function() 
{
    ...
    if (this.keys.isPressed(83)) { // DOWN
        this.p1.y = Math.min(228 - this.p1.height, this.p1.y + 4);
    } else if (this.keys.isPressed(87)) { // UP
        this.p1.y = Math.max(-228, this.p1.y - 4);
    }
 
    if (this.keys.isPressed(40)) { // DOWN
        this.p2.y = Math.min(228 - this.p2.height, this.p2.y + 4);
    } else if (this.keys.isPressed(38)) { // UP
        this.p2.y = Math.max(-228, this.p2.y - 4);
    }
    ...
    if (this.ball.vx > 0) {
        if (this.p2.x <= this.ball.x + this.ball.width &&
            ...
            if (y >= this.p2.y && y + this.ball.height <= this.p2.y + this.p2.height) {
                ...
                this.ball.yx = this.ball.yx * 1.5;
            } else if (y >= this.p2.y+256 && y + this.ball.height <= this.p2.y+256 + this.p2.height) {
                // collides with right paddle
                this.ball.x = this.p2.x - this.ball.width;
                this.ball.y = Math.floor(this.ball.y - this.ball.vy + this.ball.vy*k);
                this.ball.vx = -this.ball.vx;
        ...
        if (this.p1.x + this.p1.width >= this.ball.x) {
            ...
            if (y >= this.p1.y && y + this.ball.height <= this.p1.y + this.p1.height) {
                ...
                this.ball.vx = this.ball.vx * 1.5;
                this.ball.yx = this.ball.yx * 1.5;
            } else if (y >= this.p1.y+256 && y + this.ball.height <= this.p1.y+256 + this.p1.height) {
                // collides with the left paddle
                this.ball.x = this.p1.x + this.p1.width;
                this.ball.y = Math.floor(this.ball.y - this.ball.vy + this.ball.vy*k);
                this.ball.vx = -this.ball.vx;
...
Paddle.prototype.draw = function(p)
{
    ...
    p.fillRect(this.x, this.y+256, this.width, this.height);
};
